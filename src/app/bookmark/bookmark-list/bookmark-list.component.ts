...
export class BookmarkListComponent implements OnInit {
  ...
  handleDragOver(ev, dragItem) {
    ...
    const srcIndex = parseInt(this.srcE.dataset.id, 10);
    const desIndex = parseInt(dragItem.dataset.id, 10);
    const srcEl = this.bookmarkService.list[srcIndex];
    const desEl = this.bookmarkService.list[desIndex];
  ...
  gotoLink(index) {
    ...
    const data = this.bookmarkService.list[index];
